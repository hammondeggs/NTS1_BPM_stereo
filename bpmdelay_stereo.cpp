/*
 * File: bpmdelay_stereo.cpp
 *
 * A simple BPM sync'd pseudo-stereo delay for the NTS-1
 * 
...
void DELFX_PROCESS(float *xn, uint32_t frames)
{
   ...
   for (; x != x_e; ) 
   {
      ...
      // Pseudo-stereo - right channel gets a slightly different delay time
      // Offset the readindex by the pseudo stereo offset (10ms)
      readIndex -= PSEUDO_STEREO_OFFSET; //10ms at 48000hz sample rate works well
      
      //Since we've adjusted this value again, we must ensure we do not exceed the 0-DELAY_LINE_SIZE_MASK limits of the delay lines
      if (readIndex < 0)
      {

         // we've rolled over, add the offset
         readIndex += DELAY_LINE_SIZE_MASK;
      }
